...
/**
 * DomSimplifier - Captures a minimal, AI-friendly DOM snapshot from a Playwright page.
 *
 * Extracts only interactive/actionable elements and their ancestor chains,
 * strips noise (scripts, styles, SVGs), scrubs PII, and enforces a hard
 * character budget so the snapshot fits within AI provider token limits.
 *
 * @example
 * ```typescript
 * const simplifier = new DomSimplifier(page);
 * const html = await simplifier.getSimplifiedDOM();
 * const prompt = buildPrompt(selector, error, html.substring(0, charLimit));
 * ```
 */
